...
def get_hand_total(hand, final = false) # final = optional Boolean to indicate final hand total
  total = [0, 0]
  hand.each do |card|
    if card[2].is_a? Array # if card is an ace
...
def pretty_hand_total(hand, dealer_or_player)
  ...
  if hand_total[0] == hand_total[1]
    ...
    if blackjack?(hand)
      return "Blackjack"
    elsif dealer_or_player == 'dealer' && hand_total[1] >= 17 # dealer has to stay on soft 17 so just show the higher total
      return "#{hand_total[1]}"
    else
...
def blackjack?(hand)
  hand_total = get_hand_total(hand)
  if hand_total[0] == 21 || hand_total[1] == 21 && hand.length == 2
    return true
  end
  false
end
